...
import (
	"errors"
	"strsvc/internal/attribute/internal/db"
	"strsvc/internal/attribute/internal/evq"
	"strsvc/internal/attribute/internal/timer"
	"time"
)
...
func InitializeDB(addr, dbName, user, password string) {
	DbConfigCreator = func(args ...interface{}) db.IDbConfig {
		return db.NewMongoDbConfig(addr, dbName, user, password)
	}
